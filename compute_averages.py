...
def load_runs(domain_dir):
    ...
    runs = []
    for f in sorted(domain_dir.glob("simulation_results_*.txt")):
        data = np.loadtxt(f, delimiter=",", skiprows=3) #check if header is 3 lines
        if data.shape[1] != 2:
            raise ValueError(f"{f.name} has wrong format")
        runs.append(data)
    return runs
...
def save_averages(domain_dir, wound_dir, Lx, Ly, radius, mcs, mean, std, sem):
    ...
    with open(out_file, "w") as f:
        ...
        f.write("# Columns: mcs, mean_norm, std_norm, sem_norm\n")
